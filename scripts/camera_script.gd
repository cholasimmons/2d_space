...
@export var zoom_speed: float = 0.1 # Define the zoom speed

# Define variables
@export var screen_threshold: float = 0.95  # 15% of the screen size
@export var smoothing_speed: float = 1.0  # Adjust the smoothing speed
...
func _process(delta: float) -> void:
	if ship:
		...
		if not within_threshold:
			...
			target_position = ship.position + (difference.normalized() * radius)
		else:
		...
		global_position = global_position.slerp(target_position, smoothing_speed * delta)
